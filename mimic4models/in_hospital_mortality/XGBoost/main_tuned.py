...
def main():
    ...
    print('XGBoost does not need normalization ...')

    
    file_name = '{}.{}.xg'.format(args.period, args.features)
    ...
    "reg_lambda": uniform(5, 20),   # L2
    "reg_alpha": uniform(1, 6)
    }
    ...
    calibrated_xgb = CalibratedClassifierCV(best_xgboost, method="sigmoid", cv="prefit")
    calibrated_xgb.fit(val_X_calib, val_y_calib)
    ...
    joblib.dump(
        pipeline,
        os.path.join(args.output_dir, "xgb_pipeline.pkl")
    )

    result_dir = os.path.join(args.output_dir, 'results')
    common_utils.create_directory(result_dir)

    prediction = calibrated_xgb.predict_proba(test_X_reduced)[:, 1]
    ...
    train_probs = calibrated_xgb.predict_proba(train_X_reduced)[:, 1]
    ...
    val_probs_full = calibrated_xgb.predict_proba(val_X_thresh)[:, 1]
    ...
    val_probs = calibrated_xgb.predict_proba(val_X_thresh)[:, 1]
    ...
    plt.legend()
    plt.savefig(os.path.join(result_dir, "xgb_calibration_curve_test.png"), bbox_inches="tight")
    plt.close()
    plt.figure()
    plt.hist(y_prob, bins=50)
    plt.title("Distribution of predicted probabilities")
    plt.savefig(os.path.join(result_dir, "xgb_prob_distribution.png"))
    plt.close()
    ...
    plt.savefig(os.path.join(result_dir, "xgb_calibration_all_ICUs.png"), bbox_inches="tight")
    plt.close()
    # -----------------------------
    # ROC + PR curves
    # -----------------------------
    plot_roc_curve(y_true, y_prob, "XGB AUROC - Test Set", result_dir)
    plot_pr_curve(y_true, y_prob, "XGB AUPRC - Test Set", result_dir)

    icu_names = encoder.get_feature_names_out(["ICU"])
    feature_names = np.concatenate([feature_names, icu_names])

    print("train_X:", train_X_reduced.shape)
    print("val_X:", val_X_reduced.shape)
    print("test_X:", test_X_reduced.shape)
    ...
    plt.savefig(os.path.join(result_dir, "xgb_test_shap_summary.png"), bbox_inches="tight")
    plt.close()
    ...
    plt.savefig(os.path.join(result_dir, "xgb_test_shap_bar.png"), bbox_inches="tight")
    plt.close()
    ...
    plt.savefig(os.path.join(result_dir, "xgb_train_shap_summary.png"), bbox_inches='tight')
    plt.close()
    shap.summary_plot(shap_values_train, X_sample_train, feature_names=feature_names, plot_type="bar", show=False)
    plt.savefig(os.path.join(result_dir, "xgb_train_shap_bar.png"), bbox_inches='tight')
    plt.close()
    ...
    plt.savefig(os.path.join(result_dir, "xgb_val_shap_summary.png"), bbox_inches='tight')
    plt.close()
    shap.summary_plot(shap_values_val, val_X_thresh, feature_names=feature_names, plot_type="bar", show=False)
    plt.savefig(os.path.join(result_dir, "xgb_val_shap_bar.png"), bbox_inches='tight')
    plt.close()

    # -----------------------------
    # Metrics
    # -----------------------------
    train_metrics = print_metrics_binary(train_y, calibrated_xgb.predict_proba(train_X_reduced), "Train Confusion Matrix", result_dir, threshold=best_threshold)
    val_metrics   = print_metrics_binary(val_y_thresh, calibrated_xgb.predict_proba(val_X_thresh), "Validation Confusion Matrix", result_dir, threshold=best_threshold)
    test_metrics  = print_metrics_binary(test_y, calibrated_xgb.predict_proba(test_X_reduced), "Test Confusion Matrix", result_dir, threshold=best_threshold)
