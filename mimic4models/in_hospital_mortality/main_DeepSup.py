...
# 🔍 sanity check right after loading stats
...
print(args_dict)
class EvalAdapter(tf.keras.Model):
    """
    Wraps a multi-input model ([X, M]) into a safe single-interface model
    ...
    def predict(self, x, batch_size=None, verbose=0):
        # IMPORTANT: ensures callback compatibility
        if isinstance(x, (list, tuple)) and len(x) == 2:
            return self.base_model.predict(x, batch_size=batch_size, verbose=verbose)

        # fallback (should never happen in your setup)
        return self.base_model.predict([x, x], batch_size=batch_size, verbose=verbose)
...
if args.deep_supervision:
    ...
    def build_deep_supervision_targets(data):
        X = data[0]
        y = np.array(data[1])

        T = X.shape[1]

        # label repeated over time
        y_seq = np.repeat(y[:, None], T, axis=1)[..., None]  # (B, T, 1)

        # mask = where data is NOT all zeros
        M = np.any(X != 0, axis=-1).astype(np.float32)       # (B, T)

        return [X, M], y_seq

    train_inputs, train_y = build_deep_supervision_targets(train_raw)
    val_inputs, val_y = build_deep_supervision_targets(val_raw)
else:
    train_inputs = train_raw[0]
    train_y = np.array(train_raw[1])
    val_inputs = val_raw[0]
    val_y = np.array(val_raw[1])
...
if args.mode == 'train':
    ...
    metrics_callback = keras_utils.InHospitalMortalityMetrics(
        train_data=(train_inputs, train_y),
    ...
    if isinstance(train_inputs, list):
        print("num inputs:", len(train_inputs))
        print("X shape:", train_inputs[0].shape)
        print("M shape:", train_inputs[1].shape)
    model.fit(
        x=train_inputs,
        y=train_y,
    ...
    predictions = eval_model.predict(data, batch_size=args.batch_size, verbose=1)
    ...
    plt.title("LSTM Calibration by ICU (Test)")
    plt.xlabel("Predicted probability")
    plt.ylabel("Observed frequency")
    plt.legend(fontsize=8, ncol=2)
    plt.grid(alpha=0.3)

    plt.savefig(os.path.join(result_dir, "lstm_calibration_by_icu.png"), bbox_inches="tight")
    plt.close()
    ...
    val_data = val_raw[0]
    val_labels = np.array(val_raw[1])

    val_predictions = eval_model.predict(
        val_data,
        batch_size=args.batch_size,
        verbose=1
    )
    ...
    train_predictions = eval_model.predict(
        train_raw[0],
        batch_size=args.batch_size,
        verbose=1
    )
    ...
    plt.savefig(
        os.path.join(result_dir, "lstm_calibration_curve_test.png"),
        bbox_inches="tight"
    )
    ...
    def predict_fn(x):
        x_seq = np.repeat(x[:, np.newaxis, :], n_timesteps, axis=1)

        m_seq = np.any(x_seq != 0, axis=-1).astype(np.float32)

        return eval_model.predict([x_seq, m_seq], verbose=0).ravel()
    ...
    plt.savefig(
        os.path.join(result_dir, "lstm_shap_summary.png"),
        dpi=300
    )
    ...
    json.dump(
        train_metrics,
        open(os.path.join(result_dir, "train_lstm.json"), "w")
    )

    json.dump(
        val_metrics,
        open(os.path.join(result_dir, "val_lstm.json"), "w")
    )

    json.dump(
        test_metrics,
        open(os.path.join(result_dir, "test_lstm.json"), "w")
    )
