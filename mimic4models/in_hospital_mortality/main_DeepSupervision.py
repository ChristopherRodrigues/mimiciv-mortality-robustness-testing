...
if args.mode == 'train':
    ...
    plt.title("LSTM + DS Calibration by ICU (Test)")
    plt.xlabel("Predicted probability")
    plt.ylabel("Observed frequency")
    plt.legend(fontsize=8, ncol=2)
    plt.grid(alpha=0.3)

    plt.savefig(os.path.join(result_dir, "lstm_ds_calibration_by_icu.png"), bbox_inches="tight")
    plt.close()
    ...
    plt.savefig(
        os.path.join(result_dir, "lstm_ds_calibration_curve_test.png"),
        bbox_inches="tight"
    )
    ...
    plt.savefig(
        os.path.join(result_dir, "lstm_ds_shap_summary.png"),
        dpi=300
    )
