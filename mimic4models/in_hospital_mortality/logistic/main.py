...
from mimic4models.metrics import plot_roc_curve
from mimic4models.metrics import plot_pr_curve
...
def main():
    ...
    train_X_reduced = scaler.transform(train_X_reduced)
    val_X_reduced = scaler.transform(val_X_reduced)
    ...
    base_model = LogisticRegression(penalty=penalty,
    C=args.C,
    solver="saga",
    max_iter=5000,
    n_jobs=-1,
    ...
    base_model.fit(train_X_reduced, train_y)
    ...
    calibrated_log = CalibratedClassifierCV(estimator=base_model ,method="sigmoid",cv="prefit")
    calibrated_log.fit(val_X_calib, val_y_calib)
    pipeline = {
    "model": calibrated_log,
    ...
    joblib.dump(
        pipeline,
        os.path.join(args.output_dir, "logistic_pipeline.pkl")
    )

    result_dir = os.path.join(args.output_dir, 'results')
    common_utils.create_directory(result_dir)

    prediction = calibrated_log.predict_proba(test_X_reduced)[:, 1]
    ...
    train_probs = calibrated_log.predict_proba(train_X_reduced)[:, 1]
    ...
    val_probs_full = calibrated_log.predict_proba(val_X_thresh)[:, 1]
    ...
    val_probs = calibrated_log.predict_proba(val_X_thresh)[:, 1]
    ...
    prob_true, prob_pred = calibration_curve(
        ...
        y_prob,
        n_bins=10,
        strategy="quantile"   # good for imbalanced ICU data
    )
    ...
    plt.savefig(os.path.join(result_dir, "logreg_calibration_curve_test.png"), bbox_inches="tight")
    plt.close()
    ...
    plt.savefig(os.path.join(result_dir, "logreg_calibration_all_ICUs.png"), bbox_inches="tight")
    plt.close()

    # -----------------------------
    # ROC + PR curves
    # -----------------------------
    plot_roc_curve(test_y, prediction, "LogReg AUROC - Test Set", result_dir)
    plot_pr_curve(test_y, prediction, "LogReg AUPRC - Test Set",result_dir)
    # Build feature names
    ...
    # SHAP 
    # -----------------------------
    explainer = shap.LinearExplainer(base_model, train_X_reduced)
    ...
    plt.savefig(os.path.join(result_dir, "logreg_test_shap_summary.png"), bbox_inches='tight')
    plt.close()
    shap.summary_plot(shap_values, X_sample, feature_names=feature_names, plot_type="bar", show=False)
    plt.savefig(os.path.join(result_dir, "logreg_test_shap_bar.png"), bbox_inches='tight')
    plt.close()
    ...
    plt.savefig(os.path.join(result_dir, "logreg_train_shap_summary.png"), bbox_inches='tight')
    plt.close()
    shap.summary_plot(shap_values_train, X_sample_train, feature_names=feature_names, plot_type="bar", show=False)
    plt.savefig(os.path.join(result_dir, "logreg_train_shap_bar.png"), bbox_inches='tight')
    plt.close()
    ...
    plt.savefig(os.path.join(result_dir, "logreg_val_shap_summary.png"), bbox_inches='tight')
    plt.close()
    shap.summary_plot(shap_values_val, val_X_thresh, feature_names=feature_names, plot_type="bar", show=False)
    plt.savefig(os.path.join(result_dir, "logreg_val_shap_bar.png"), bbox_inches='tight')
    plt.close()
    ...
    train_metrics = print_metrics_binary(train_y, calibrated_log.predict_proba(train_X_reduced), "Train Confusion Matrix", result_dir,threshold=best_threshold)
    val_metrics   = print_metrics_binary(val_y_thresh, calibrated_log.predict_proba(val_X_thresh), "Val Confusion Matrix", result_dir, threshold=best_threshold)
    test_metrics  = print_metrics_binary(test_y, calibrated_log.predict_proba(test_X_reduced), "Test Confusion Matrix", result_dir, threshold=best_threshold)
