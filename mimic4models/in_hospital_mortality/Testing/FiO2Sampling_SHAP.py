...
# FIO2 POPULATIONS
#
# FiO2 is extracted as the FIRST recorded value for each
# patient.
#
# Reference:
#     0.21 <= FiO2 < 0.30
...
# ============================================================
# FIO2 EXTRACTION / POPULATION SAMPLING
# ============================================================
...
def extract_fio2(X_raw, header):
    ...
    for patient in X_raw:
        ...
        if len(valid) > 0:
            ...
            # First recorded FiO2 reflects the initial
            # oxygenation support at admission.
            fio2.append(valid[0])
    ...
    return np.asarray(fio2, dtype=float)
...
def compare_shap(
    ...
    high_df.to_csv(
        os.path.join(
            out_dir,
            f"mean_abs_shap_highfio2_{suffix}.csv"
        ),
        index=False
    )
...
def run_classical_shap(
    ...
    np.save(
        os.path.join(
            out_dir,
            f"shap_highfio2_{model_name}.npy"
        ),
        shap_high
    )
...
# RenalSampling_SHAP.py / WeightSampling_SHAP.py.
#
# Reference and high-FiO2 populations are independent
...
def run_deep_shap(
    ...
    plt.savefig(
        os.path.join(
            out_dir,
            f"summary_highfio2_{suffix}.png"
        ),
        dpi=300,
        bbox_inches="tight"
    )
...
def main(
    ...
    (
        ...
        icu,
        fio2,
        fio2_min=REFERENCE_FIO2_MIN,
        fio2_max=REFERENCE_FIO2_MAX
    )
    ...
    if (
        ...
        raise RuntimeError(
            "One of the FiO2 populations "
            "is empty. Check the Fraction "
            "inspired oxygen column and "
            "thresholds."
        )
    ...
    # WeightSampling_SHAP.py.
    #
    # The FiO2 array comes from the raw test data and
    ...
    if len(snapshot_icu) != len(
        ...
        raise RuntimeError(
            ...
            f"row count ({len(fio2)}) "
            "used for FiO2 extraction. "
            "The two are assumed to be in "
            "the same patient order."
        )

    # ========================================================
    # DEEP FIO2 POPULATIONS
    # ========================================================
