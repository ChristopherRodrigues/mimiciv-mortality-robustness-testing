...
def plot_weight_kde(
    ...
    w_high_weight = np.asarray(w_high_weight, dtype=float)
    ...
    w_reference = w_reference[np.isfinite(w_reference)]
    w_high_weight = w_high_weight[np.isfinite(w_high_weight)]
    ...
    # to prevent extreme weight outliers from compressing
    ...
    plt.plot(
        ...
        y,
        linewidth=2,
        label=f"Entire population (n={len(weight)})"
    )
    ...
    plt.title(
        ...
        "Reference and High-Weight Cohorts",
        fontsize=14
    )
    ...
    print(
        f"99th percentile weight: "
        f"{np.percentile(weight, 99):.2f} kg"
    )
...
def run_experiment(
    ...
    return {
        ...
        "high_weight": high_weight_metrics
    }
# =========================
...
def run_all_models(
    ...
    for name, path in models.items():
        ...
        res = run_experiment(
            ...
            X_high_weight=X_high_weight,
            y_high_weight=y_high_weight,
            icu_high_weight=icu_high_weight,

            out_dir=out_dir
        )

        reference = res["reference"]
        high_weight = res["high_weight"]
        ...
        results.append({
            ...
            # High weight cohort
            "HighWeight_AUROC": high_weight["AUROC"],
            "HighWeight_AUPRC": high_weight["AUPRC"],
            "HighWeight_Brier": high_weight["Brier"],
            "HighWeight_ECE": high_weight["ECE"],
            "HighWeight_MCE": high_weight["MCE"],

            # Difference (High weight − Reference)
            "Delta_AUROC": high_weight["AUROC"] - reference["AUROC"],
            "Delta_AUPRC": high_weight["AUPRC"] - reference["AUPRC"],
            "Delta_Brier": high_weight["Brier"] - reference["Brier"],
            "Delta_ECE": high_weight["ECE"] - reference["ECE"],
            "Delta_MCE": high_weight["MCE"] - reference["MCE"]
        })

    df = pd.DataFrame(results)

    df.to_csv(
        os.path.join(out_dir, "weight_population_summary.csv"),
        index=False
    )
...
def run_all_models_deep(
    ...
    for name, model_path in models.items():
        ...
        p_high_weight = model.predict(X_high_weight)

        if isinstance(p_high_weight, list):
            p_high_weight = p_high_weight[0]

        p_high_weight = np.asarray(p_high_weight).ravel()
        ...
        high_weight_metrics = evaluate_deep(
            p_high_weight,
            y_high_weight
        )
        ...
        results.append({
            ...
            "HighWeight_AUROC": high_weight_metrics["AUROC"],
            "HighWeight_AUPRC": high_weight_metrics["AUPRC"],
            "HighWeight_Brier": high_weight_metrics["Brier"],
            "HighWeight_ECE": high_weight_metrics["ECE"],
            "HighWeight_MCE": high_weight_metrics["MCE"],

            "Delta_AUROC": high_weight_metrics["AUROC"] - reference_metrics["AUROC"],
            "Delta_AUPRC": high_weight_metrics["AUPRC"] - reference_metrics["AUPRC"],
            "Delta_Brier": high_weight_metrics["Brier"] - reference_metrics["Brier"],
            "Delta_ECE": high_weight_metrics["ECE"] - reference_metrics["ECE"],
            "Delta_MCE": high_weight_metrics["MCE"] - reference_metrics["MCE"]
        })

    df = pd.DataFrame(results)

    df.to_csv(
        os.path.join(out_dir, "weight_deep_summary.csv"),
        index=False
    )
...
def main(return_df=False, cli_args=None):
    ...
    weights = extract_weight(
        X_raw,
        test_ret["header"]
    )

    print("Patients:", len(weights))
    print("Valid weights:", np.sum(np.isfinite(weights)))

    print(pd.Series(weights).describe())
    pd.Series(weights).describe(percentiles=[0.25,0.5,0.75,0.9,0.95])
    ...
    X_reference, y_reference, icu_reference, w_reference = sample_weight_population(
        X_base,
        y,
        icu,
        weights,
    ...
    X_high_weight, y_high_weight, icu_high_weight, w_high_weight = sample_weight_population(
        X_base,
        y,
        icu,
        weights,
    ...
    print(f"Mean weight = {np.mean(w_reference):.2f} kg")
    print(f"Range = {w_reference.min():.1f} - {w_reference.max():.1f} kg")
    print(f"Mortality = {np.mean(y_reference):.3f}")

    print("\nHigh-weight cohort")
    print(f"N = {len(y_high_weight)}")
    print(f"Mean weight = {np.mean(w_high_weight):.2f} kg")
    print(f"Range = {w_high_weight.min():.1f} - {w_high_weight.max():.1f} kg")
    print(f"Mortality = {np.mean(y_high_weight):.3f}")
    os.makedirs("weight_results", exist_ok=True)
    ...
    final_df = df
    final_df.to_csv(
        "weight_results/weight_summary.csv",
        index=False
    )
    print("\nCombined Results:")
    print(final_df)
