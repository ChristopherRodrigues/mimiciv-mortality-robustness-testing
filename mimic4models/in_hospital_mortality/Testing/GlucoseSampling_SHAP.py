...
# PATHS
#
# Identical structure to WeightSampling_SHAP.py / the renal
# (Creatinine) SHAP script / MICU_SHAP.py / SICU_SHAP.py.
# ============================================================
...
# GLUCOSE POPULATIONS
#
# Glucose is extracted as the FIRST recorded glucose
# measurement for each patient.
#
# Reference:
#     70 <= glucose < 140  (normoglycemic)
#
# High glucose:
#     glucose >= 200  (hyperglycemic)
#
# These are two separate real patient populations.
# There is no artificial perturbation.
# ============================================================

REFERENCE_GLUCOSE_MIN = 70
REFERENCE_GLUCOSE_MAX = 140

HIGH_GLUCOSE_MIN = 200
...
# ============================================================
# GLUCOSE EXTRACTION / POPULATION SAMPLING
# ============================================================
...
def extract_glucose(X_raw, header):
    ...
    for patient in X_raw:
        ...
        if len(valid) > 0:
            ...
            # First recorded glucose measurement
...
# ============================================================
# GLUCOSE DISTRIBUTION KDE
#
# Kept from the original glucose script as a supplementary
# visualization — not part of the SHAP pipeline itself.
# ============================================================
...
def plot_glucose_kde(
    ...
    if len(g_all) == 0:
        ...
        print("No valid glucose values available for KDE plot.")

        return

    kde = gaussian_kde(g_all, bw_method="scott")

    lower_bound = max(0.0, g_all.min() - 10)

    upper_bound = np.percentile(g_all, 99) + 10
    ...
    reference_mask = (
        (x >= REFERENCE_GLUCOSE_MIN) & (x < REFERENCE_GLUCOSE_MAX)
    )

    plt.fill_between(
        x[reference_mask], y[reference_mask], alpha=0.30,
        label=(
            f"Reference glucose "
            f"({REFERENCE_GLUCOSE_MIN}\u2013<{REFERENCE_GLUCOSE_MAX} mg/dL, "
            f"n={len(g_reference)})"
        )
    )

    high_mask = (x >= HIGH_GLUCOSE_MIN)

    plt.fill_between(
        x[high_mask], y[high_mask], alpha=0.30,
        label=(
            f"High glucose (\u2265{HIGH_GLUCOSE_MIN} mg/dL, "
            f"n={len(g_high_glucose)})"
        )
    )

    for boundary in [
        REFERENCE_GLUCOSE_MIN, REFERENCE_GLUCOSE_MAX, HIGH_GLUCOSE_MIN
    ]:

        plt.axvline(boundary, linestyle="--", linewidth=1.2, alpha=0.7)
    ...
    print(f"99th percentile glucose: {np.percentile(g_all, 99):.2f} mg/dL")

    plt.tight_layout()

    output_path = os.path.join(out_dir, "glucose_population_kde.png")
...
#
# Exactly the same transformation logic as the renal
# (Creatinine) SHAP script / MICU_SHAP.py.
# ============================================================
...
def print_shap_results(
    ...
    top10_high = set(high_df.head(10)["Feature"])

    overlap = top10_reference & top10_high
    ...
    print("Top features increasing under high glucose:")
    ...
    if len(increasing) == 0:
        ...
        increasing_display = increasing[
            ...
            "Mean_Abs_SHAP_Perturbed": "HighGlucose"
        })
...
# SHAP COMPARISON
#
# Same generalized comparison used by the renal
# (Creatinine) SHAP script / WeightSampling_SHAP.py.
# ============================================================
...
def compare_shap(
    ...
    if deep:
        ...
        high_df = mean_abs_shap_deep(high_shap, feature_names)

    else:

        reference_df = mean_abs_shap(reference_shap, feature_names)

        high_df = mean_abs_shap(high_shap, feature_names)
    ...
    high_df.to_csv(
        os.path.join(out_dir, f"mean_abs_shap_highglucose_{suffix}.csv"),
        index=False
    )
    ...
    comparison = pd.merge(
        ...
        high_df[["Feature", "Mean_Abs_SHAP", "Rank"]],
    ...
    plt.xlabel("Reference SHAP Rank")

    plt.ylabel("High-glucose SHAP Rank")

    plt.title(f"Glucose SHAP Rank Stability\nSpearman \u03c1 = {rho:.3f}")
    ...
    pd.DataFrame({
        ...
        "HighGlucose_Top10": [", ".join(high_df.head(10)["Feature"])],
    ...
    plt.title(
        "Glucose Feature Attribution Change\n"
        f"Reference ({REFERENCE_GLUCOSE_MIN}\u2013{REFERENCE_GLUCOSE_MAX}) "
        f"\u2192 High glucose (\u2265{HIGH_GLUCOSE_MIN})"
    )
    ...
    return (comparison, rho, p_value, reference_df, high_df)
...
def run_classical_shap(
    ...
    X_high_transformed = transform_classical(X_high, icu_high, pipeline)
    ...
    if model_name == "XGBoost":
        ...
        shap_high = normalize_shap_values(
            explainer.shap_values(X_high_transformed)
        )
        ...
        n_high_explain = min(100, len(X_high_transformed))
        ...
        shap_high = normalize_shap_values(
            explainer.shap_values(
                X_high_transformed[:n_high_explain], silent=True
            )
        )
    ...
    np.save(
        os.path.join(out_dir, f"shap_highglucose_{model_name}.npy"),
        shap_high
    )

    suffix = model_name.lower().replace(" ", "_")

    (comparison, rho, p, reference_df, high_df) = compare_shap(
        shap_ref, shap_high, feature_names, out_dir, suffix, deep=False
    )
...
#
# Same architecture as the renal (Creatinine) SHAP script /
...
# DEEP SHAP
#
# Same population-comparison implementation as the renal
# (Creatinine) SHAP script / WeightSampling_SHAP.py.
#
# Reference and high-glucose populations are independent
...
def run_deep_shap(
    ...
    if X_reference.shape[1:] != X_high.shape[1:]:
        ...
        raise ValueError(
            f"{model_name}: reference and high-glucose tensors must "
    ...
    print(
        ...
        f"reference: {X_reference.shape}, high-glucose: {X_high.shape}"
    )
    ...
    print("SHAP high-glucose:", high_sample.shape)
    ...
    print("Calculating high-glucose SHAP...")
    ...
    print("High-glucose SHAP shape:", shap_high.shape)
    ...
    np.save(
        os.path.join(out_dir, f"shap_highglucose_{suffix}.npy"), shap_high
    )
    ...
    plt.savefig(
        os.path.join(out_dir, f"summary_highglucose_{suffix}.png"),
        dpi=300, bbox_inches="tight"
    )
...
def main(return_df=False, cli_args=None):
    ...
    icu = np.asarray([icu_map.get(n, "UNKNOWN") for n in names])

    # ========================================================
    # GLUCOSE EXTRACTION
    # ========================================================

    glucoses = extract_glucose(X_raw, test_ret["header"])

    valid_glucose = glucoses[np.isfinite(glucoses)]

    print()

    print("Valid glucose values:", len(valid_glucose))
    print("Mean glucose:", np.mean(valid_glucose))
    print("Min glucose:", np.min(valid_glucose))
    print("Max glucose:", np.max(valid_glucose))
    ...
    # ========================================================
    # GLUCOSE POPULATION SAMPLING
    # ========================================================

    (
        X_reference_classical, y_reference, icu_reference, g_reference
    ) = sample_glucose_population(
        X_base, y, icu, glucoses,
        glucose_min=REFERENCE_GLUCOSE_MIN, glucose_max=REFERENCE_GLUCOSE_MAX
    )

    (
        X_high_classical, y_high, icu_high, g_high
    ) = sample_glucose_population(
        X_base, y, icu, glucoses, glucose_min=HIGH_GLUCOSE_MIN
    )
    ...
    print(
        ...
        f"({REFERENCE_GLUCOSE_MIN} <= glucose < {REFERENCE_GLUCOSE_MAX}):",
        len(X_reference_classical)
    )

    print(
        f"High-glucose population (glucose >= {HIGH_GLUCOSE_MIN}):",
    ...
    if len(X_reference_classical) == 0 or len(X_high_classical) == 0:
        ...
        raise RuntimeError(
            "One of the glucose populations is empty. Check the "
            "Glucose column and thresholds."
        )
    ...
    # ========================================================
    # KDE DISTRIBUTION PLOT (supplementary, not part of SHAP)
    # ========================================================
    ...
    try:
        ...
        plot_glucose_kde(
            ...
            g_high_glucose=g_high,
            out_dir=args.output_dir
        )

    except Exception as exc:

        print(f"WARNING: glucose KDE plot failed: {exc}")
    ...
    # DEEP SNAPSHOT
    #
    # Same approach as the renal (Creatinine) SHAP script /
    # WeightSampling_SHAP.py.
    #
    # The glucose array comes from the raw test data and is
    # assumed to have the same patient ordering/count as the
    # deep snapshot.
    # ========================================================
    ...
    if len(snapshot_icu) != len(glucoses):

        raise RuntimeError(
            "Deep snapshot row count "
            f"({len(snapshot_icu)}) does not match the raw test-set "
            f"row count ({len(glucoses)}) used for glucose extraction. "
            "The two are assumed to be in the same patient order."
        )

    # ========================================================
    # DEEP GLUCOSE POPULATIONS
    # ========================================================

    (
        X_deep_reference, _, icu_deep_reference, _
    ) = sample_glucose_population(
        X_norm, snapshot_y, snapshot_icu, glucoses,
        glucose_min=REFERENCE_GLUCOSE_MIN, glucose_max=REFERENCE_GLUCOSE_MAX
    )
    ...
    (
        ...
        X_norm, snapshot_y, snapshot_icu, glucoses,
        glucose_min=HIGH_GLUCOSE_MIN
    )

    print()

    print(
        f"Deep reference population "
        f"({REFERENCE_GLUCOSE_MIN} <= glucose < {REFERENCE_GLUCOSE_MAX}):",
        len(X_deep_reference)
    )

    print(
        f"Deep high-glucose population (glucose >= {HIGH_GLUCOSE_MIN}):",
        len(X_deep_high)
    )
    ...
    if len(X_deep_reference) == 0 or len(X_deep_high) == 0:
        ...
        raise RuntimeError(
            "One of the deep-learning glucose populations is empty."
        )
    ...
    summary.to_csv(
        os.path.join(args.output_dir, "glucose_shap_summary.csv"),
        index=False
    )
