...
# MICU UNIT NAME
# =========================
MICU_UNIT = "Medical Intensive Care Unit (MICU)"
...
def extract_feature_series_by_icu(X_raw, feature_names, feature_query, icu, target="MICU"):
    feature_names = np.asarray(feature_names)
    idx = None
    for i, f in enumerate(feature_names):
        if feature_query.lower() in str(f).lower():
    ...
    for patient, unit in zip(X_raw, icu):
        if target == "MICU":
            keep = str(unit) == MICU_UNIT
        elif target == "NON_MICU":
            keep = str(unit) != MICU_UNIT
        else:
            raise ValueError("target must be 'MICU' or 'NON_MICU'")
        if not keep:
            continue
        for row in patient:
            try:
...
# =========================
# MICU PERTURBATION (RAW)
# =========================

def apply_micu_perturbation_raw(X, icu, severity, feature_names):
    """
    Apply MICU respiratory failure perturbations to raw MIMIC time series.

    Perturbations:
        Minimal  — Mild hypoxaemia / increased respiratory support
        Moderate — Moderate hypoxaemia / significantly increased FiO2
        Severe   — Severe hypoxaemia / critical respiratory failure
    """
    X = list(X)
    icu = np.asarray(icu)
    feature_names = np.asarray(feature_names)
    ...
    spo2_i = find_feature(["oxygen saturation"])
    ...
    for patient, unit in zip(X, icu):
        ...
        spo2_drop = np.random.uniform(*params["spo2_drop"])
        fio2_inc  = np.random.uniform(*params["fio2_increase"])
        ...
        for row in patient:
            ...
            if row[spo2_i] != "":
                ...
                if np.isfinite(v) and 50 <= v <= 100:
                    row[spo2_i] = str(np.clip(v * (1 - spo2_drop), 50, 100))

            # FiO2 — percentage increase
            if row[fio2_i] != "":
                v = safe_float(row[fio2_i])
                if np.isfinite(v) and 0.21 <= v <= 1.0:
                    row[fio2_i] = str(np.clip(v * (1 + fio2_inc), 0.21, 1.0))

            # Heart Rate — Gaussian noise
            if row[hr_i] != "":
                v = safe_float(row[hr_i])
                if np.isfinite(v) and 20 <= v <= 250:
                    row[hr_i] = str(np.clip(v + np.random.normal(0, params["hr_noise"]), 20, 250))

            # Respiratory Rate — Gaussian noise
            if row[rr_i] != "":
                v = safe_float(row[rr_i])
                if np.isfinite(v) and 2 <= v <= 80:
                    row[rr_i] = str(np.clip(v + np.random.normal(0, params["rr_noise"]), 2, 80))

    return X

# =========================
# MICU PERTURBATION (TENSOR)
# =========================

def apply_micu_perturbation_tensor(X, icu, severity, feature_names):
    """
    Apply MICU perturbation to denormalized deep-learning tensors.
    ...
    micu_mask = np.array([str(x) == MICU_UNIT for x in icu])
    if micu_mask.sum() == 0:
        return X
    if X.ndim != 3:
        raise RuntimeError("MICU tensor perturbation only supports (N,T,F) tensors.")

    n = micu_mask.sum()
    T = X.shape[1]

    spo2_drop = np.random.uniform(*params["spo2_drop"], size=n)
    fio2_inc  = np.random.uniform(*params["fio2_increase"], size=n)

    # Oxygen Saturation — percentage drop
    X[micu_mask, :, spo2_i] *= (1 - spo2_drop[:, None])
    X[:, :, spo2_i] = np.clip(X[:, :, spo2_i], 50, 100)

    # FiO2 — percentage increase
    X[micu_mask, :, fio2_i] *= (1 + fio2_inc[:, None])
    X[:, :, fio2_i] = np.clip(X[:, :, fio2_i], 0.21, 1.00)

    # Heart Rate — Gaussian noise
    X[micu_mask, :, hr_i] += np.random.normal(0, params["hr_noise"], size=(n, T))
    X[:, :, hr_i] = np.clip(X[:, :, hr_i], 20, 250)

    # Respiratory Rate — Gaussian noise
    X[micu_mask, :, rr_i] += np.random.normal(0, params["rr_noise"], size=(n, T))
    X[:, :, rr_i] = np.clip(X[:, :, rr_i], 2, 80)
...
def run_all_models(models, X_base, X_pert, y, icu, feature_names, severity="Moderate", out_dir="."):
    ...
    df.to_csv(os.path.join(out_dir, "micu_summary.csv"), index=False)
    return df
...
def run_all_models_deep(models: dict,
    ...
    for name, model_path in models.items():
        ...
        np.random.seed(42)
        X_pert_raw = apply_micu_perturbation_tensor(
    ...
    df = pd.DataFrame(results)
    df.to_csv(os.path.join(out_dir, "micu_deep_summary.csv"), index=False)
    return df
...
def main(return_df=False, cli_args=None):
    ...
    # PERTURB raw
    np.random.seed(42)
    X_raw_pert = apply_micu_perturbation_raw(
        copy.deepcopy(X_raw), icu, severity, test_ret['header']
    )

    # Sanity checks
    icu_arr   = np.array(icu)
    micu_mask = np.array([str(x) == MICU_UNIT for x in icu_arr])
    print("MICU patients:", micu_mask.sum())
    print("Total patients:", len(micu_mask))
    print("Fraction:", micu_mask.mean())

    for feat_query in ["oxygen saturation", "fraction inspired oxygen", "heart rate", "respiratory rate"]:
        try:
    ...
    # KDE plots — MICU only and Non-MICU control
    plot_features = [
        ("Oxygen Saturation",       "oxygen saturation"),
        ("Fraction Inspired O2",    "fraction inspired oxygen"),
        ("Heart Rate",              "heart rate"),
        ("Respiratory Rate",        "respiratory rate"),
    ]

    result_dir = r"C:/Users/chris/Thesis/mimic4-benchmarks/micu_results/"
    os.makedirs(result_dir, exist_ok=True)

    for label, target in [("MICU", "MICU"), ("Non-MICU control", "NON_MICU")]:
        ...
        plt.suptitle(f"MICU Perturbation KDEs — {severity} — {label}", fontsize=14)
        plt.tight_layout()
        fname = f"micu_kde_{severity.lower()}_{target.lower()}.png"
    ...
    final_df.to_csv(f"micu_results/{sev}_summary.csv", index=False)
    print("\nCombined Results:")
    print(final_df)
