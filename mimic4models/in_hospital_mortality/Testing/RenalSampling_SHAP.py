...
# PATHS
#
# Identical structure to WeightSampling_SHAP.py /
# MICU_SHAP.py / SICU_SHAP.py.
# ============================================================
...
# RENAL POPULATIONS
#
# Creatinine is extracted as the MAXIMUM observed creatinine
# for each patient.
#
# Reference:
#     0.6 <= creatinine < 1.3
...
REFERENCE_CREATININE_MIN = 0.6
REFERENCE_CREATININE_MAX = 1.3

HIGH_CREATININE_MIN = 2.0
...
# ============================================================
# CREATININE EXTRACTION / POPULATION SAMPLING
# ============================================================

def extract_creatinine(X_raw, header):

    creatinine_idx = header.index("Creatinine")

    creatinine = []

    for patient in X_raw:

        values = np.asarray(patient)[:, creatinine_idx]
        ...
        if len(valid) > 0:
            ...
            # Peak creatinine reflects the maximum observed
            # renal impairment during the available stay.
...
# SHAP COMPARISON
#
# Same generalized comparison used by WeightSampling_SHAP.py.
# ============================================================
...
def compare_shap(
    ...
    high_df.to_csv(
        os.path.join(
            out_dir,
            f"mean_abs_shap_highcreatinine_{suffix}.csv"
        ),
        index=False
    )
    ...
    plt.ylabel(
        "High-creatinine SHAP Rank"
    )

    plt.title(
        f"Renal SHAP Rank Stability\n"
        f"Spearman ρ = {rho:.3f}"
    )
    ...
    plt.title(
        "Renal Feature Attribution Change\n"
        "Reference (0.6–1.3) → High creatinine (≥2.0)"
    )
...
def run_classical_shap(
    ...
    np.save(
        os.path.join(
            out_dir,
            f"shap_highcreatinine_{model_name}.npy"
        ),
        shap_high
    )
...
#
# Same architecture as WeightSampling_SHAP.py.
# ============================================================
...
# WeightSampling_SHAP.py.
#
# Reference and high-creatinine populations are independent
...
def run_deep_shap(
    ...
    if (
        ...
        raise ValueError(
            ...
            f"high-creatinine tensors must "
    ...
    print(
        ...
        f"reference: {X_reference.shape}, "
        f"high-creatinine: {X_high.shape}"
    )
    ...
    # --------------------------------------------------------
    # High-creatinine SHAP
    # --------------------------------------------------------

    print(
        "Calculating high-creatinine SHAP..."
    )
    ...
    np.save(
        os.path.join(
            out_dir,
            f"shap_highcreatinine_{suffix}.npy"
        ),
        shap_high
    )
    ...
    plt.savefig(
        os.path.join(
            out_dir,
            f"summary_highcreatinine_{suffix}.png"
        ),
        dpi=300,
        bbox_inches="tight"
    )
...
def main(
    ...
    print(
        ...
        len(valid_creatinine)
    )

    print(
        "Mean peak creatinine:",
        np.mean(valid_creatinine)
    )

    print(
        "Min peak creatinine:",
        np.min(valid_creatinine)
    )

    print(
        "Max peak creatinine:",
        np.max(valid_creatinine)
    )
    ...
    (
        ...
        icu,
        creatinine,
        creat_min=REFERENCE_CREATININE_MIN,
        creat_max=REFERENCE_CREATININE_MAX
    )
    ...
    if (
        ...
        raise RuntimeError(
            "One of the creatinine populations "
            "is empty. Check the Creatinine "
    ...
    # DEEP SNAPSHOT
    #
    # Same approach as WeightSampling_SHAP.py.
    #
    # The creatinine array comes from the raw test data and
    ...
    if len(snapshot_icu) != len(
        ...
        raise RuntimeError(
            ...
            f"row count ({len(creatinine)}) "
            "used for creatinine extraction. "
            "The two are assumed to be in "
            "the same patient order."
        )

    # ========================================================
    # DEEP RENAL POPULATIONS
    # ========================================================
    ...
    (
        ...
        creat_min=(
            REFERENCE_CREATININE_MIN
        ),

        creat_max=(
            REFERENCE_CREATININE_MAX
        )
    )
    ...
    print(
        "Deep high-creatinine population "
        "(creatinine >= 2.0):",
        len(X_deep_high)
    )
    ...
    if (
        ...
        raise RuntimeError(
            ...
            "creatinine populations is empty."
        )
