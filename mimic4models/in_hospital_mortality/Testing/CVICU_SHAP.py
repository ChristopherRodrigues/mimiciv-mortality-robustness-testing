...
# ============================================================
# RAW CVICU PERTURBATION
# ============================================================

def apply_cvicu_perturbation_raw(
    X,
    icu,
    severity,
    feature_names
    ...
    dbp_i, has_dbp = try_find(
        ["Diastolic blood pressure"]
    )

    has_bp = has_sbp and has_dbp
    ...
    for patient, unit in zip(
        ...
        # expands deviations around the patient mean
        # ----------------------------------------------------

        if (
            has_bp
            and
            params["bp_variability"] != 1.0
        ):
...
def apply_cvicu_perturbation_tensor(
    X,
    icu,
    severity,
    feature_names,
    ...
    # --------------------------------------------------------
    # Percentage application (fixed, non-random)
    # --------------------------------------------------------
    ...
    # --------------------------------------------------------
    # Absolute application (fixed, non-random)
    # --------------------------------------------------------
    ...
    if (
        ...
        for feat_i, lo, hi in [
            ...
            bp_mean = (
                (bp * valid).sum(axis=1, keepdims=True)
                / count
            )
...
def compare_shap(
    ...
    plt.title(
        "CVICU Feature Attribution Change\n"
        "Baseline → CVICU Post-Cardiac Surgery Perturbation"
    )
...
def main(
    ...
    for model_name, pipeline_path in (
        ...
        results[model_name] = (
            run_classical_shap(
                ...
                X_reference=X_cvicu_base,

                X_perturbed=X_cvicu_pert,

                icu_reference=icu_cvicu,

                icu_perturbed=icu_cvicu,
    ...
    icu_deep_cvicu = snapshot_icu[
        snapshot_cvicu_mask
    ]

    print()
    print(
        "Deep-learning CVICU patients:",
        len(X_deep_cvicu)
    )

    if len(X_deep_cvicu) == 0:
    ...
    for model_name in deep_models:
        ...
        X_deep_cvicu_pert = (
            apply_cvicu_perturbation_tensor(
        ...
        if (
            ...
            raise RuntimeError(
                ...
                f"shape changed: "
                f"{X_deep_cvicu.shape} -> "
                f"{X_deep_cvicu_pert.shape}"
            )
        ...
        results[model_name] = (
            run_deep_shap(
                ...
                X_reference=X_deep_cvicu,

                X_perturbed=X_deep_cvicu_pert,
