...
def main():
    ...
    train_X_reduced = np.hstack([train_X_reduced, train_icu_enc])
    val_X_reduced   = np.hstack([val_X_reduced, val_icu_enc])
    test_X_reduced  = np.hstack([test_X_reduced, test_icu_enc])

    print('Random Forest does not need normalization ...')

    file_name = '{}.{}.rf'.format(args.period, args.features)

    randomforest = RandomForestClassifier( n_jobs = -1, random_state=42, bootstrap = True)

    # Hyperparameter search space
    param_dist = {
    'n_estimators': randint(100, 500), # 100–500 trees
    'max_depth': [None] + list(range(5, 31, 5)), # None or 5,10,...30
    'min_samples_split': randint(2, 20), # min 2–20 samples to split
    'min_samples_leaf': randint(1, 10), # min 1–10 samples per leaf
    ...
    random_search.fit(train_X_reduced, train_y)

    print("Best parameters:", random_search.best_params_)
    print("Best CV score (ROC-AUC):", random_search.best_score_)

    best_rf = random_search.best_estimator_

    val_X_calib, val_X_thresh, val_y_calib, val_y_thresh = train_test_split(
    val_X_reduced, val_y, test_size=0.5, random_state=42, stratify=val_y
    )

    calibrated_rf = CalibratedClassifierCV(best_rf, method="sigmoid", cv="prefit")
    calibrated_rf.fit(val_X_calib, val_y_calib)
    ...
    "feature_names_before_filtering": np.array(build_feature_names(train_feature_header)),

    "feature_names_after_variance": feature_names_variance,

    "feature_names_after_filtering": feature_names_final
    }

    joblib.dump(
        pipeline,
        os.path.join(args.output_dir, "rf_pipeline.pkl")
    )

    result_dir = os.path.join(args.output_dir, 'results')
    common_utils.create_directory(result_dir)

    prediction = calibrated_rf.predict_proba(test_X_reduced)[:, 1]
    ...
    train_probs = calibrated_rf.predict_proba(train_X_reduced)[:, 1]
    ...
    val_probs_full = calibrated_rf.predict_proba(val_X_thresh)[:, 1]
    ...
    val_probs = calibrated_rf.predict_proba(val_X_thresh)[:, 1]
    ...
    plt.legend()
    plt.savefig(os.path.join(result_dir, "rf_calibration_curve_test.png"), bbox_inches="tight")
    plt.close()
    plt.figure()
    plt.hist(y_prob, bins=50)
    plt.title("Distribution of predicted probabilities")
    plt.savefig(os.path.join(result_dir, "rf_prob_distribution.png"))
    plt.close()
    ...
    plt.savefig(os.path.join(result_dir, "rf_calibration_all_ICUs.png"), bbox_inches="tight")
    plt.close()
    # -----------------------------
    # ROC + PR curves
    # -----------------------------
    plot_roc_curve(y_true, y_prob, "RF AUROC - Test Set", result_dir)
    plot_pr_curve(y_true, y_prob, "RF AUPRC - Test Set", result_dir)
    ...
    # sample for speed 
    sample_idx = np.random.choice(len(test_X_reduced), min(2000, len(test_X_reduced)), replace=False)
    X_sample = test_X_reduced[sample_idx]
    ...
    plt.savefig(os.path.join(result_dir, "rf_test_shap_summary.png"), bbox_inches="tight")
    plt.close()
    ...
    plt.savefig(os.path.join(result_dir, "rf_test_shap_bar.png"), bbox_inches="tight")
    plt.close()

    # Compute SHAP values for train split

    sample_idx_train = np.random.choice(len(train_X_reduced), 2000, replace=False)
    X_sample_train = train_X_reduced[sample_idx_train]
    ...
    plt.savefig(os.path.join(result_dir, "rf_train_shap_summary.png"), bbox_inches='tight')
    plt.close()
    shap.summary_plot(shap_values_train, X_sample_train, feature_names=feature_names, plot_type="bar", show=False)
    plt.savefig(os.path.join(result_dir, "rf_train_shap_bar.png"), bbox_inches='tight')
    plt.close()

    # Compute SHAP values for val split
    
    shap_values_val = explainer.shap_values(val_X_thresh)

    if isinstance(shap_values_val, list):
        shap_values_val = shap_values_val[1]
    elif shap_values_val.ndim == 3:
        shap_values_val = shap_values_val[:, :, 1]
    shap.summary_plot(shap_values_val, val_X_thresh, feature_names=feature_names, show=False)
    plt.savefig(os.path.join(result_dir, "rf_val_shap_summary.png"), bbox_inches='tight')
    plt.close()
    shap.summary_plot(shap_values_val, val_X_thresh, feature_names=feature_names, plot_type="bar", show=False)
    plt.savefig(os.path.join(result_dir, "rf_val_shap_bar.png"), bbox_inches='tight')
    plt.close()

    # -----------------------------
    # Metrics
    # -----------------------------
    train_metrics = print_metrics_binary(train_y, calibrated_rf.predict_proba(train_X_reduced), "Train Confusion Matrix", result_dir, threshold=best_threshold)
    val_metrics   = print_metrics_binary(val_y_thresh, calibrated_rf.predict_proba(val_X_thresh), "Validation Confusion Matrix", result_dir, threshold=best_threshold)
    test_metrics  = print_metrics_binary(test_y, calibrated_rf.predict_proba(test_X_reduced), "Test Confusion Matrix", result_dir, threshold=best_threshold)
